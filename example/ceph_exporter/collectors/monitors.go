...
type MonitorCollector struct {
	...
	HostUpNum    prometheus.Gauge
	HostTotalNum prometheus.Gauge
}
...
func NewMonitorCollector(conn Conn, cluster string, logger *logrus.Logger) *MonitorCollector {
	...
	return &MonitorCollector{
		...
		HostUpNum: prometheus.NewGauge(
			prometheus.GaugeOpts{
				Namespace:   cephNamespace,
				Name:        "host_up_num",
				Help:        "host_up_num for ceph cluster",
				ConstLabels: labels,
			},
		),
		HostTotalNum: prometheus.NewGauge(
			prometheus.GaugeOpts{
				Namespace:   cephNamespace,
				Name:        "host_total_num",
				Help:        "host_total_num for ceph cluster",
				ConstLabels: labels,
			},
		),
	}
...
func (m *MonitorCollector) collect() error {
	HostTotalNum, HostUpNum := GetHostUpNum()
	m.HostUpNum.Set(float64(HostUpNum))
	m.HostTotalNum.Set(float64(HostTotalNum))
...
func GetHostUpNum() (HostTotalNum, HostUpNum int64) {
	...
	for _, v := range ips {
		...
		go func(v string) {
			...
			defer pinger.Stop()
			pinger.Timeout = time.Duration(time.Duration(PingOutTime) * time.Millisecond)
			pinger.Count = 3
